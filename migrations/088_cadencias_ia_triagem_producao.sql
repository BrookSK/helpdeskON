-- =====================================================================
-- 088_cadencias_ia_triagem_producao.sql
-- ---------------------------------------------------------------------
-- Incorpora a TRIAGEM POR IA (ChatGPT) nas 3 cadências ON Solu, com os
-- prazos de PRODUÇÃO. Reescreve o grafo (UPDATE) de cada sequência:
--
--   Ao RESPONDER (qualquer toque): move card → "Respondeu" → IA (decisão)
--     * INTERESSE     → confirma (e-mail/WhatsApp) → move "Qualificado"
--                       → tag "interessado" → encerra
--     * SEM INTERESSE → confirma/agradece → REMOVE da lista (unsubscribe)
--                       → move "Perdido" → tag "sem interesse" → encerra
--   Ao ESGOTAR sem resposta: tag "sem resposta" → score -10
--                       → move "Sem resposta" → encerra
--
-- Requer os blocos 'ai' e 'unsubscribe' do SequenceEngine e a openai_api_key.
-- Colunas usadas: Respondeu, Qualificado, Perdido, Sem resposta.
-- Idempotente. Para TESTE de 10 min use o 089.
-- =====================================================================

-- ---------------------------------------------------------------------
-- (A) Templates de confirmação (e-mail já vem do 087; recria idempotente).
--     Cria também as versões de WhatsApp (Fluxo 3).
-- ---------------------------------------------------------------------
...
'{{primeiro_nome}}, que ótimo! Obrigado pelo retorno.

Vou organizar as próximas informações e um especialista da ON Solutions entra em contato para entender melhor o cenário da {{empresa}} e sugerir os próximos passos.'
...
'{{primeiro_nome}}, entendido, obrigado pelo retorno.

Removi o seu contato da nossa lista de acompanhamento para não enviar mais mensagens. Se quiser retomar no futuro, é só me chamar por aqui. Sucesso à {{empresa}}!'
WHERE NOT EXISTS (SELECT 1 FROM message_templates WHERE name = 'ON Solu · Triagem Sem Interesse (Remoção) WA');

-- ---------------------------------------------------------------------
-- FLUXO 1 (Só E-mail) — prazos reais + triagem IA
-- ---------------------------------------------------------------------
UPDATE email_sequences SET graph = JSON_OBJECT(
    ...
    'nodes', JSON_ARRAY(
        ...
        -- Triagem por IA ao responder (branch compartilhado)
        ...
        JSON_OBJECT('id','ia','type','ai','x',760,'y',360,'nextYes','pos_mail','nextNo','neg_mail','data', JSON_OBJECT(
            ...
            'prompt','Analise a última resposta do lead {{primeiro_nome}} (empresa {{empresa}}) e o histórico. decision=true SOMENTE se demonstrou INTERESSE (quer conversar, receber material, reunião ou seguir). decision=false se sem interesse, pediu para parar, não é o momento ou já tem fornecedor. Em dúvida real, decision=false.')),
        ...
        JSON_OBJECT('id','neg_unsub','type','unsubscribe','x',960,'y',620,'next','neg_move','data', JSON_OBJECT('reason','Sem interesse (classificado pela IA)')),
...
-- ---------------------------------------------------------------------
-- FLUXO 2 (E-mail + WhatsApp) — prazos reais + triagem IA
-- ---------------------------------------------------------------------
UPDATE email_sequences SET graph = JSON_OBJECT(
    'start', 'e1',
    'nodes', JSON_ARRAY(
        ...
        JSON_OBJECT('id','ia','type','ai','x',760,'y',360,'nextYes','pos_mail','nextNo','neg_mail','data', JSON_OBJECT(
            'mode','decision','model','gpt-4o-mini',
            'prompt','Analise a última resposta do lead {{primeiro_nome}} (empresa {{empresa}}) e o histórico (e-mail e WhatsApp). decision=true SOMENTE se demonstrou INTERESSE (quer conversar, receber material, reunião ou seguir). decision=false se sem interesse, pediu para parar, não é o momento ou já tem fornecedor. Em dúvida real, decision=false.')),
...
-- ---------------------------------------------------------------------
-- FLUXO 3 (Só WhatsApp) — prazos reais + triagem IA (confirmação via WhatsApp)
-- ---------------------------------------------------------------------
UPDATE email_sequences SET graph = JSON_OBJECT(
    'start', 'wa1',
    'nodes', JSON_ARRAY(
        ...
        JSON_OBJECT('id','ia','type','ai','x',760,'y',360,'nextYes','pos_wa','nextNo','neg_wa','data', JSON_OBJECT(
            'mode','decision','model','gpt-4o-mini',
            'prompt','Analise a última resposta do lead {{primeiro_nome}} (empresa {{empresa}}) e o histórico de WhatsApp. decision=true SOMENTE se demonstrou INTERESSE (quer conversar, receber material, reunião ou seguir). decision=false se sem interesse, pediu para parar, não é o momento ou já tem fornecedor. Em dúvida real, decision=false.')),
