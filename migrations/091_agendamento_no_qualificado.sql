-- =====================================================================
-- 091_agendamento_no_qualificado.sql  (opcional, complementar ao 088)
-- ---------------------------------------------------------------------
-- Insere o bloco de AGENDAMENTO no ramo "interessado" das cadências ON Solu:
-- após a IA classificar INTERESSE (mover para "Qualificado" + tag), envia o
-- link público de agendamento por e-mail e/ou WhatsApp. Ao o lead escolher o
-- horário, o sistema cria o evento no Google Meet e notifica por e-mail e WhatsApp.
--
-- Requer: bloco 'schedule' (SequenceEngine) e tabela agenda_booking_links (090).
-- Idempotente: regrava o grafo por nome. Aplica aos 3 fluxos (F1, F2 e F3):
-- no ramo positivo, após a confirmação da IA, o 'pos_tag' aponta para 'sched'
-- (type=schedule), que envia a mensagem + o link com os horários e depois encerra.
-- =====================================================================
...
UPDATE email_sequences SET graph = JSON_OBJECT(
    ...
    'nodes', JSON_ARRAY(
        ...
        JSON_OBJECT('id','pos_tag','type','tag','x',560,'y',740,'next','sched','data', JSON_OBJECT('label','interessado','color','#28a745')),
        -- NOVO: envia o link de agendamento ao lead interessado
        JSON_OBJECT('id','sched','type','schedule','x',560,'y',860,'next','done','data', JSON_OBJECT(
...
-- ---------------------------------------------------------------------
-- FLUXO 2 (E-mail + WhatsApp) — agendamento após interesse
-- ---------------------------------------------------------------------
UPDATE email_sequences SET graph = JSON_OBJECT(
    'start', 'e1',
    'nodes', JSON_ARRAY(
...
-- ---------------------------------------------------------------------
-- FLUXO 3 (Só WhatsApp) — agendamento após interesse (confirmação via WhatsApp)
-- ---------------------------------------------------------------------
UPDATE email_sequences SET graph = JSON_OBJECT(
    'start', 'wa1',
    'nodes', JSON_ARRAY(
        ...
        JSON_OBJECT('id','pos_tag','type','tag','x',560,'y',740,'next','sched','data', JSON_OBJECT('label','interessado','color','#28a745')),
        JSON_OBJECT('id','sched','type','schedule','x',560,'y',860,'next','done','data', JSON_OBJECT(
            'channel','whatsapp','duration',45,
            'title','Reunião com a ON Solutions Brasil',
            'message','{{primeiro_nome}}, que ótimo! Escolha o melhor dia e horário para uma conversa rápida (online): {{link_agendamento}}')),
