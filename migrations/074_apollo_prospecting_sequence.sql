...
SELECT
    ...
    JSON_OBJECT(
        'start', 'send1',
        'nodes', JSON_ARRAY(
            JSON_OBJECT('id','send1','type','send','next','wait1','data', JSON_OBJECT(
                'ab_enabled', true,
            ...
            JSON_OBJECT('id','wait1','type','wait','next','cond1','data', JSON_OBJECT('amount',3,'unit','days')),
            JSON_OBJECT('id','cond1','type','condition','nextYes','done','nextNo','send2','data', JSON_OBJECT('kind','replied')),
            JSON_OBJECT('id','send2','type','send','next','wait2','data', JSON_OBJECT(
                'template_id', (SELECT id FROM message_templates WHERE name='Apollo · Follow-up 1')
            )),
            JSON_OBJECT('id','wait2','type','wait','next','cond2','data', JSON_OBJECT('amount',3,'unit','days')),
            JSON_OBJECT('id','cond2','type','condition','nextYes','done','nextNo','revealph','data', JSON_OBJECT('kind','replied')),
            JSON_OBJECT('id','revealph','type','reveal_phone','next','waitph','data', JSON_OBJECT()),
            JSON_OBJECT('id','waitph','type','wait','next','wa1','data', JSON_OBJECT('amount',1,'unit','days')),
            JSON_OBJECT('id','wa1','type','whatsapp','next','wait3','data', JSON_OBJECT(
                'template_id', (SELECT id FROM message_templates WHERE name='Apollo · WhatsApp 1º Contato')
            )),
            JSON_OBJECT('id','wait3','type','wait','next','cond3','data', JSON_OBJECT('amount',2,'unit','days')),
            JSON_OBJECT('id','cond3','type','condition','nextYes','done','nextNo','task1','data', JSON_OBJECT('kind','replied')),
            JSON_OBJECT('id','task1','type','tag','next','send3','data', JSON_OBJECT('label','call_task_sem_resposta')),
            JSON_OBJECT('id','send3','type','send','next','wait4','data', JSON_OBJECT(
                'template_id', (SELECT id FROM message_templates WHERE name='Apollo · Follow-up Final')
            )),
            JSON_OBJECT('id','wait4','type','wait','next','cond4','data', JSON_OBJECT('amount',3,'unit','days')),
            JSON_OBJECT('id','cond4','type','condition','nextYes','done','nextNo','cold','data', JSON_OBJECT('kind','replied')),
            JSON_OBJECT('id','cold','type','tag','next','done','data', JSON_OBJECT('label','cold_no_response')),
            JSON_OBJECT('id','done','type','end','data', JSON_OBJECT())
        )
    ),
