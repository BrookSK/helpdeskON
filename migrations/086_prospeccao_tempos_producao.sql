-- =====================================================================
-- 086_prospeccao_tempos_producao.sql  (REVERTE o 085)
-- ---------------------------------------------------------------------
-- Restaura os prazos REAIS de produção das 3 cadências "ON Solu", após o
-- teste com esperas de 10 minutos (085). Mantém a estrutura idêntica
-- (083 + 084), incluindo o nó 'moved' (move → "Respondeu").
--
-- Prazos de produção:
--   Fluxo 1 (e-mail):  3, 4, 5, 6, 7, 7 dias  (D0/3/7/12/18/25/32)
--   Fluxo 2 (misto):   2, 2, 3, 3, 4 dias     (D0/2/4/7/10/14)
--   Fluxo 3 (WhatsApp):3, 4, 4, 4 dias        (D0/3/7/11/15)
-- =====================================================================

-- ---------------------------------------------------------------------
-- FLUXO 1 (Só E-mail) — prazos reais
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- FLUXO 2 (E-mail + WhatsApp) — prazos reais
-- ---------------------------------------------------------------------
UPDATE email_sequences SET graph = JSON_OBJECT(
    'start', 'e1',
    'nodes', JSON_ARRAY(
...
-- ---------------------------------------------------------------------
-- FLUXO 3 (Só WhatsApp) — prazos reais
-- ---------------------------------------------------------------------
UPDATE email_sequences SET graph = JSON_OBJECT(
    'start', 'wa1',
    'nodes', JSON_ARRAY(
