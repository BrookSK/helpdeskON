-- =====================================================================
-- 084_prospeccao_move_respondeu.sql  (complementar ao 083)
-- ---------------------------------------------------------------------
-- Ajusta as 3 cadências "ON Solu" para, ao detectar RESPOSTA do lead,
-- MOVER o card para a coluna "Respondeu" antes de encerrar a sequência.
--
-- Faz isso reescrevendo o grafo (UPDATE) de cada sequência: adiciona um nó
-- 'moved' (type=move, column_name='Respondeu' → done) e reaponta todos os
-- ramos 'nextYes' das condições "respondeu?" de 'done' para 'moved'.
--
-- A movimentação para "Em prospecção" ao INICIAR a sequência é feita no
-- código (SequenceEngine::enroll), não depende de migration.
--
-- Idempotente: pode reexecutar (apenas regrava o grafo). Só afeta sequências
-- que já existem (criadas pelo 083).
-- =====================================================================
