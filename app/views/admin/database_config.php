...
<div class="<?= $user ? 'main-content' : 'container py-5' ?>">
    <div class="top-bar">
        <div>
            ...
            <small class="text-muted">Dados de conexão MySQL</small>
        </div>
    </div>
            ...
            <form action="<?= baseUrl('settings/saveDatabase') ?>" method="POST">
                <div class="row g-3">
                    <div class="col-8">
                        ...
                        <input type="text" name="host" class="form-control form-control-sm" value="<?= escape($config['host'] ?? 'localhost') ?>">
                    </div>
                    <div class="col-4">
                        <label class="form-label fw-medium small">Porta</label>
                        <input type="text" name="port" class="form-control form-control-sm" value="<?= escape($config['port'] ?? '3306') ?>">
                    </div>
                    <div class="col-12">
                        <label class="form-label fw-medium small">Nome do Banco</label>
                        <input type="text" name="database" class="form-control form-control-sm" value="<?= escape($config['database'] ?? '') ?>">
                    </div>
                    <div class="col-sm-6">
                        <label class="form-label fw-medium small">Usuário</label>
                        <input type="text" name="username" class="form-control form-control-sm" value="<?= escape($config['username'] ?? '') ?>">
                    </div>
                    <div class="col-sm-6">
                        <label class="form-label fw-medium small">Senha</label>
                        <input type="password" name="password" class="form-control form-control-sm" value="<?= escape($config['password'] ?? '') ?>">
                    </div>
                    <div class="col-12 mt-2 d-flex gap-2">
                        <button type="submit" class="btn btn-primary btn-sm px-4"><i class="bi bi-check-lg"></i> Salvar</button>
                        <?php if ($user): ?>
                        <a href="<?= baseUrl('settings') ?>" class="btn btn-outline-secondary btn-sm">Voltar</a>
                        <?php endif; ?>
                    </div>
                </div>
            </form>
