...
class CronController extends Controller
{
    ...
    public function index()
    {
        $this->json([
            'endpoints' => [
                ...
                'GET /cron/verifyScheduledPosts?token=XXX' => 'Verifica posts agendados que deveriam ter sido publicados',
            ],
    ...
    /**
     * GET /cron/verifyScheduledPosts?token=XXX
     * Verifica posts que foram agendados localmente e cuja due_at já passou.
     * Consulta a API do Buffer para checar se realmente foram publicados.
     * Atualiza o status local e loga warnings para posts com falha.
     *
     * Recomendação de cron: a cada 15 minutos.
     *   [asterisco]/15 * * * * curl -s "https://seudominio.com/cron/verifyScheduledPosts?token=SEU_TOKEN" > /dev/null
     */
    public function verifyScheduledPosts()
    {
        $this->validateToken();
        @set_time_limit(120);
        ...
        // Busca posts locais que deveriam ter sido publicados (due_at já passou há 10+ minutos)
        $overdue = $bufferData->getOverdueScheduledPosts(10);

        if (empty($overdue)) {
            $this->json(['success' => true, 'message' => 'Nenhum post pendente de verificação.', 'checked' => 0]);
        }

        $stats = ['checked' => 0, 'confirmed_sent' => 0, 'failed' => 0, 'still_pending' => 0];
        $failures = [];

        // Agrupa por buffer_account_id para reutilizar a mesma instância de API
        $grouped = [];
        foreach ($overdue as $post) {
            $accId = $post['buffer_account_id'] ?: 'legacy';
            $grouped[$accId][] = $post;
        }

        foreach ($grouped as $accId => $posts) {
            // Determina a API key
            $apiKey = null;
            if ($accId !== 'legacy') {
                $acc = $bufferAccounts->findById($accId);
                if ($acc && !empty($acc['api_key'])) {
                    $apiKey = $acc['api_key'];
                }
            }
            $api = new BufferApi($apiKey);
            if (!$api->hasKey()) {
                Logger::warning('Buffer verifyScheduledPosts: sem API key para conta', ['buffer_account_id' => $accId]);
                continue;
            }

            foreach ($posts as $post) {
                $stats['checked']++;
                $res = $api->getPost($post['buffer_post_id']);
                $node = $res['data']['post'] ?? null;

                if (!$node) {
                    // Post não encontrado na API — pode ter sido deletado
                    $bufferData->updatePostStatus($post['buffer_post_id'], 'error_not_found');
                    $stats['failed']++;
                    $failures[] = [
                        'buffer_post_id' => $post['buffer_post_id'],
                        'channel' => $post['channel_name'] ?? $post['channel_id'],
                        'service' => $post['channel_service'] ?? null,
                        'due_at' => $post['due_at'],
                        'reason' => 'Post não encontrado na API do Buffer (pode ter sido deletado)',
                    ];
                    Logger::warning('Buffer post not found after schedule', [
                        'buffer_post_id' => $post['buffer_post_id'],
                        'channel' => $post['channel_name'] ?? null,
                        'due_at' => $post['due_at'],
                    ]);
                    continue;
                }

                $remoteStatus = strtolower($node['status'] ?? '');

                if ($remoteStatus === 'sent') {
                    // Publicou com sucesso — atualiza localmente
                    $extra = [];
                    if (!empty($node['sentAt'])) {
                        $extra['sent_at'] = date('Y-m-d H:i:s', strtotime($node['sentAt']));
                    }
                    if (!empty($node['externalLink'])) {
                        $extra['external_link'] = $node['externalLink'];
                    }
                    $bufferData->updatePostStatus($post['buffer_post_id'], 'sent', $extra);
                    $stats['confirmed_sent']++;

                } elseif (in_array($remoteStatus, ['error', 'failed', 'needs_approval'])) {
                    // Falhou no Buffer
                    $bufferData->updatePostStatus($post['buffer_post_id'], 'error');
                    $stats['failed']++;
                    $failures[] = [
                        'buffer_post_id' => $post['buffer_post_id'],
                        'channel' => $post['channel_name'] ?? $post['channel_id'],
                        'service' => $post['channel_service'] ?? null,
                        'due_at' => $post['due_at'],
                        'reason' => 'Buffer reportou status: ' . $remoteStatus,
                    ];
                    Logger::warning('Buffer post failed to publish', [
                        'buffer_post_id' => $post['buffer_post_id'],
                        'channel' => $post['channel_name'] ?? null,
                    ...
                    // Ainda pending/scheduled/sending — pode estar processando
                    $stats['still_pending']++;

                    // Se já passou mais de 1 hora e ainda está pending, marca como problema
                    $dueTs = strtotime($post['due_at']);
                    if ($dueTs && (time() - $dueTs) > 3600) {
                        $bufferData->updatePostStatus($post['buffer_post_id'], 'error_timeout');
                        $stats['failed']++;
                        $stats['still_pending']--;
                        $failures[] = [
                            'buffer_post_id' => $post['buffer_post_id'],
                            ...
                            'service' => $post['channel_service'] ?? null,
                            'due_at' => $post['due_at'],
                            'reason' => 'Post ainda não publicado após 1h do horário agendado (status remoto: ' . $remoteStatus . ')',
                        ];
                        Logger::warning('Buffer post timeout - not published after 1h', [
                            'buffer_post_id' => $post['buffer_post_id'],
                            'channel' => $post['channel_name'] ?? null,
                            'remote_status' => $remoteStatus,
                            'due_at' => $post['due_at'],
        ...
        // Loga resumo
        if (!empty($failures)) {
            Logger::warning('Buffer verifyScheduledPosts: posts com falha detectados', [
